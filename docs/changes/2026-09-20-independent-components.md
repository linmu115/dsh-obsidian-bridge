# DSH Obsidian Bridge 独立组件架构升级

对应候选版本：0.4.1-rc2.4。依据用户已确认的 2026-09-20 架构文档与本次「按文档对各个插件进行架构升级」指令。

## 已实现

本实例设置页管理多个 Vault；不注册 Maintenance 绑定页；可选 CLI 缺失时不暴露可执行 CLI 工具。

技术入口：[实现](../../src/binding-settings.tsx)。本轮只有源码和候选产物，尚未安装到实际实例；不以单元测试替代真实数据验收。
