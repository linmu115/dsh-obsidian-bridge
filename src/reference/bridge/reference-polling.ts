...
export interface ReferencePollingOptions {
  ...
  isVisible?: () => boolean;
  maxPending?: number;
  ephemeralNavigation?: (message: BridgeAction) => boolean;
}
...
export function startReferencePolling(
  ...
  const cycle = async (): Promise<void> => {
    ...
    try {
      ...
      const latestNavigation = ordered.findLast(entry => entry.message.type === "deep-link" && options.ephemeralNavigation?.(entry.message) && (entry.message.referenceId === undefined || !deleting.has(entry.message.referenceId)));
      for (const entry of ordered) {
        ...
        try {
          if (applied.has(entry.cursor)) outcome = "handled";
            ...
            // Ordinary navigation is one-shot: coalesce stale clicks and never
            // replay a failed location after reconnect or an acknowledgement retry.
            if (entry === latestNavigation) {
              try { await apply(entry.message, abort.signal); }
              catch (error) { if (stopped) break; options.onActionError?.(error, entry.message); }
            }
            outcome = "handled";
        ...
        } else { retries.delete(entry.cursor); applied.delete(entry.cursor); }
