...
export function registerBridgeBusinessPage(service:BusinessPageService,lifecycle:ObsidianBridgeLifecycle,identity:DshInstanceIdentity):()=>void {
 ...
 return service.register({namespace:'obsidian-bridge',providerId:'vault-bindings',
  async snapshot(){
   ...
   const sections:Snapshot['sections']=[{id:'overview',title:'Obsidian 连接',kind:'summary',text:`已发现 ${vaults.length} 个 Vault。绑定由各 Vault 保存；此页面只显示状态并提交操作。更多 Vault 可在 DSH 的 Obsidian 面板管理。`},
    ...
    {id:'binding-actions',title:'绑定管理',kind:'actions',actions:vaults.filter(actionable).slice(0,16).map(vault=>({id:actionId(vault),label:`${vault.state==='bound'?'解除绑定':vault.state==='foreign'?'改绑到当前实例':'绑定当前实例'}：${vault.displayName}`.slice(0,200),expectedRevision:vault.binding.revision,fields:[]}))}];
