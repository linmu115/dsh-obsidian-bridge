...
import { startReferencePolling } from "../src/reference/bridge/reference-polling.ts";
import type { BridgeActionPage } from "../src/reference/bridge/http-client.ts";
...
function setup(page: BridgeActionPage) {
  ...
  const bridge = { nextActions: vi.fn(async () => page), acknowledgeDeepLink: vi.fn(async (_actionId: string, _signal?: AbortSignal) => {}), acknowledgeAction: vi.fn(async () => {}) };
...
it("retries only the acknowledgement after successful delivery", async () => {
 const fixture=setup({queueId:"one",cursor:1,actions:[deletion(1)]});
 fixture.bridge.acknowledgeAction.mockRejectedValueOnce(Error("lost ack"));
 const apply=vi.fn(async()=>"handled" as const);const poll=startReferencePolling(fixture.bridge,apply,fixture.options);await poll.firstCycle;
 poll.retry();await vi.waitFor(()=>expect(fixture.bridge.acknowledgeAction).toHaveBeenCalledTimes(2));expect(apply).toHaveBeenCalledOnce();poll.stop();
});

it("coalesces owned ordinary navigation without stealing a foreign surface click", async () => {
 const navigation=(cursor:number,targetSurfaceId="mine")=>({cursor,message:{protocolVersion:1 as const,type:"deep-link" as const,actionId:`n${cursor}`,sessionId:"s",anchorId:"a",targetSurfaceId}});
 const fixture=setup({queueId:"one",cursor:3,actions:[navigation(1),navigation(2),navigation(3,"foreign")]});
 const apply=vi.fn(async(action:any)=>action.targetSurfaceId==="foreign"?"ignored" as const:"handled" as const);
 const poll=startReferencePolling(fixture.bridge,apply,{...fixture.options,ephemeralNavigation:action=>action.type==="deep-link"&&action.targetSurfaceId==="mine"});await poll.firstCycle;
 expect(apply.mock.calls.map(([action])=>action.actionId)).toEqual(["n2","n3"]);expect(fixture.bridge.acknowledgeDeepLink.mock.calls.map(([id])=>id)).toEqual(["n1","n2"]);poll.stop();
});
it("acknowledges failed ordinary navigation once and retries only a lost ack", async () => {
 const fixture=setup({queueId:"one",cursor:1,actions:[{cursor:1,message:{protocolVersion:1,type:"deep-link",actionId:"n1",sessionId:"s",anchorId:"a"}}]});fixture.bridge.acknowledgeDeepLink.mockRejectedValueOnce(Error("lost ack"));
 const apply=vi.fn(async()=>{throw Error("missing session")});const onActionError=vi.fn();const poll=startReferencePolling(fixture.bridge,apply,{...fixture.options,onActionError,ephemeralNavigation:()=>true});await poll.firstCycle;poll.retry();await vi.waitFor(()=>expect(fixture.bridge.acknowledgeDeepLink).toHaveBeenCalledTimes(2));expect(apply).toHaveBeenCalledOnce();poll.stop();
});
