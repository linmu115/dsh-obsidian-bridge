...
import { createBridgeHttpClient } from "../src/reference/bridge/http-client.ts";
import type { ReferencePollingHandle } from "../src/reference/bridge/reference-polling.ts";
import { apply as applyClient } from "../src/reference/client.ts";
import { createReferenceDeleteActionHandler } from "../src/reference/bridge/reference-delete-actions.ts";
import { ReferenceDeleteCommitV2Schema, type ReferenceDeleteRequestV2 } from "../src/reference/protocol.ts";
  ...
  it.each(["pending", "sent"] as const)("lets a browser consumer drain an already-deleted %s relation without a Core cleanup job", async (scope) => {
    ...
    const context = {
      ...
      obsidianBridgeLifecycle: {
        ...
        transport: source.bridge,
        registerActionHandler(_id: string, handler: { handle: Parameters<typeof startReferencePolling>[1] }) {
          polling = startReferencePolling(source.bridge, handler.handle); return () => polling!.stop();
        },
